...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
        ...
        private static Vector3 nextBoundPoint
        {
            ...
            {
                ++_pinBoundPointIdx;
                if (_pinBoundPointIdx >= _pinBoundPoints[_pinBoundLayerIdx].Count) _pinBoundPointIdx = 1;
                return _pinBoundPoints[_pinBoundLayerIdx][_pinBoundPointIdx];
            }
        }
        ...
        private static void SetPinValues(Quaternion additionRotation)
        {
            ...
            var bounds = BoundsUtils.GetBoundsRecursive(prefab.transform, additionRotation);
            ...
            var centerToPivotOnPlane = Vector3.ProjectOnPlane(centerToPivot,
                additionRotation * Vector3.up);

            var pointRotation = Quaternion.Inverse(additionRotation);

            var pivotTocenter = Vector3.zero;
            ...
            for (int y = -1; y <= 1; y += 1)
            {
                ...
                var newY = -y * ySign * halfSize.y + centerToPivot.y;
                ...
                var center = pointRotation * new Vector3(0f, -y * ySign * halfSize.y, 0f) + centerToPivot;
                ...
                {
                    var point = pointRotation * Vector3.Scale(!isSprite
                            ? new Vector3(n.x, -y * ySign, n.y)
                            : new Vector3(n.x, n.y, -y * ySign), halfSize) + centerToPivot;
        ...
        public static void UpdatePinValues()
        {
            ...
            var fromUpToNormalRotation = Quaternion.FromToRotation(additionalRotation* Vector3.up, _pinHit.normal);
            ...
            {
                if (fromUpToNormalEulerRounded.y == 180) _pinBoundLayerIdx = 2;
                else _pinBoundLayerIdx = 0;
            }
        ...
        private static void PinPreview(Camera camera)
        {
            ...
            BrushSettings brushSettings = strokeItem.settings;
            if (PinManager.settings.overwriteBrushProperties) brushSettings = PinManager.settings.brushSettings;
            if(!brushSettings.isAsset2D && !brushSettings.rotateToTheSurface && _prevPinHitNormal != _pinHit.normal)
            {
            ...
            if (brushSettings.rotateToTheSurface && !PinManager.settings.flattenTerrain)
            {
                ...
                if (_pinHit.collider == null && strokeItem.settings.isAsset2D)
        ...
        private static void PinInput(UnityEditor.SceneView sceneView)
        {
            ...
            else if (PWBSettings.shortcuts.pinSurfDist.Check())
            {
                ...
                if (combi.mouseEvent == PWBMouseCombination.MouseEvents.SCROLL_WHEEL) _pinDistanceFromSurface += Event.current.delta.y * 0.04f;
        ...
        private static void DrawPinHandles(Matrix4x4 rootToWorld, Color color)
        {
            ...
            for (int i = 0; i < _pinBoundPoints[layerIdx].Count; ++i)
            {
                ...
                pos = Vector3.Scale((Vector3)(rootToWorld * (_pinOffset - _pinBoundPoints[layerIdx][i])),
